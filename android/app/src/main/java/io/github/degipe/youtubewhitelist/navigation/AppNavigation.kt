...
import io.github.degipe.youtubewhitelist.feature.parent.ui.channelsearch.ChannelSearchScreen
import io.github.degipe.youtubewhitelist.feature.parent.ui.channelsearch.ChannelSearchViewModel
...
fun AppNavigation(
    ...
    LaunchedEffect(sideInput, navController) {
        sideInput.navigationActions.collect { action ->
            when (action) {
                SideInputAction.Back -> {
                    val route = navController.currentBackStackEntry?.destination?.route.orEmpty()
                    if (!route.contains("KidHome")) navController.popBackStack()
                }
                SideInputAction.Home -> navController.popBackStack<Route.KidHome>(inclusive = false)
                else -> Unit
            }
        }
    }
    ...
    NavHost(
        ...
        composable<Route.ParentDashboard> {
            ...
            ParentDashboardScreen(
                ...
                onOpenChannelSearch = { profileId ->
                    navController.navigate(Route.ChannelSearch(profileId))
                },
        ...
        composable<Route.ChannelSearch> { backStackEntry ->
            val route = backStackEntry.toRoute<Route.ChannelSearch>()
            val viewModel: ChannelSearchViewModel =
                hiltViewModel<ChannelSearchViewModel, ChannelSearchViewModel.Factory> { factory ->
                    factory.create(route.profileId)
                }
            ChannelSearchScreen(
                viewModel = viewModel,
